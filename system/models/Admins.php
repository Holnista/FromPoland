<?php
	...
	{
		protected $json = ['personal', 'employment', 'permissions'];
		...
		function open()
		{
			...
			//----- Unique key based on updated column
			$this->bean->data['key'] = md5(implode('', [$this->table, $this->bean->id, $this->bean->password]));
		}
		...
		function get()
		{
			...
			//----- Get name variant
			$this->bean->data['variant'] = R::getCell(sprintf("SELECT `variant` FROM `%s` WHERE `name` = '%s'", 
			TABLE_NAMES, $this->bean->personal['name'])); if(!$this->bean->data['variant'])
			$this->bean->data['variant'] = $this->bean->personal['name'];
		...
		{
			//----- Remove temp file
			unlink(path(PATH_UPLOADS, 'temp', $this->bean->data['key']));
