	...
	class Images extends \Model
	{
		...
		function get()
		{
			...
			if(in_array('image', $data) || !$data)
			{
				...
				$path = path(PATH_IMAGES, $this->bean->table, $this->bean->pid); if(file_exists(path($path, $this->bean->file)) && is_file(path($path, $this->bean->file)))
		...
		{
			//----- Remove image
			$path = path(PATH_IMAGES, $this->bean->table, $this->bean->pid);
