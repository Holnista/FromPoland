...
{block "content"}
	{include "../addons/topbar.tpl" topbar=['title' => ['icon' => 'desktop', 'content' => $t['PAGES']], 'options' => [($count) ? 'search' : '', 'add']]}
	{if $pages}
		<table class="table table-bordered table-hover table-sm" id="table">
			...
			<tbody>
				{foreach from = array_values($pages) item = page}
					<tr>
						<td class="text-right text-muted d-none d-md-table-cell">{$page['id']}</td>
						<td class="text-left" data-order="{clear($page['title'])}">
							<div class="row align-items-center no-gutters flex-nowrap">
								<div class="col-auto d-none d-xl-block mr-2">
									{if !$page['file']}<img src="{$page['data']['thumb']}" class="img-thumbnail" width="48">{else}
										<a href="{$page['data']['image']}" data-fancybox="pages" data-caption="{$page['title']}">
											<img src="{$page['data']['thumb']}" alt="{$page['title']}" class="img-thumbnail p-0" width="48">
										</a>
									{/if}
								</div>
								<div class="col">
									<div class="d-flex align-items-center">
										{if $page['parent'] && !$smarty.get.word}
											<span class="mover" style="width: {50*count(parents(TABLE_PAGES, $page['id']))}px;"></span>
										{/if}
										<div>
											<b>{$page['title']}</b><br>
											<small class="text-muted">{$page['alias']}</small>
										</div>
									</div>
								</div>
								<div class="col-auto d-none d-sm-block ml-auto text-right">
									{if $page['place']}<span class="badge {if $page['place'] == 'home'}
									badge-dark{else}badge-success{/if}">{$t[strtoupper($page['place'])]}</span>
									{else}<span class="badge badge-dark">{$t['NONE']}</span>{/if}
								</div>
							</div>
						</td>
						<td class="text-center d-none d-sm-table-cell" data-order="{$page['active']}">
							<div class="custom-control custom-switch custom-switch-success">
								<input type="checkbox" id="page[{$page['id']}][active]" 
								data-toggle-active="{path(TABLE_PAGES, $page['id'])}"
								class="custom-control-input" {if $page['active']}checked{/if} readonly>
								<label class="custom-control-label" for="page[{$page['id']}][active]"></label>
							</div>
						</td>
						<td class="text-center">
							<div class="dropdown">
								...
								<div class="dropdown-menu dropdown-menu-right">
									<a href="{$base->PATH}/edit/{$page['id']}" class="dropdown-item">
										<span>{$t['EDIT']}</span><i class="fa fa-pencil text-success"></i>
									</a>
									<a href="{$base->PATH}/duplicate/{$page['id']}" class="dropdown-item">
										<span>{$t['DUPLICATE']}</span><i class="fa fa-copy text-primary"></i>
									</a>
									<a href="#" data-toggle="modal" data-target="#remove{$page['id']}" class="dropdown-item">
...
{block "modal"}
	{if $pages}
		{foreach from = array_values($pages) item = page}
			<div class="modal fade" id="remove{$page['id']}">
				<div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
					<div class="modal-content">
						...
						<div class="modal-body text-center py-5">
							<p class="m-0">{sprintf($t['RECORDS_REMOVE_TITLE'], $page['title'])}</p>
						...
						<div class="modal-footer justify-content-between">
							...
							<a href="{$base->PATH}/remove/{$page['id']}" class="btn rounded btn-danger">
								<span>{$t['REMOVE']}</span>
							</a>
						</div>
					</div>
